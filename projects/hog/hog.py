...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        total += number
        num_rolls -= 1
    return total if not sow_sad else 1
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    score = abs(opponent_score // 10 % 10 - player_score % 10) * 3
    return max(score, 1)
    # END PROBLEM 2
...
def sus_points(score):
    ...
    if num_factors(score) == 3 or num_factors(score) == 4:
        # increase to the closest prime number greater than current score
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(0, goal):
        for opponent_score in range(0, goal):
            if strategy(score, opponent_score) != number_of_dice:
                return False
    return True
    # END PROBLEM 7
